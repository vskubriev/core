 ...
 */
class LogSettingsControllerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testSetLogLevel($level, $inRange) {
		...
		if ($inRange) {
			$expectedResponse = ['level' => $level, 'status' => 'success'];
		} else {
			$expectedResponse = ['message' => 'log-level out of allowed range', 'status' => 'error'];
		}
